...
jintArray NewIntArray(JNIEnv *env,int len){
...
JIntArray::JIntArray(JNIEnv *env, jintArray array) : env(env), array(array){
    this->len = env->GetArrayLength(array);
}

void JIntArray::set(int index, int value){
    if (index < 0 || index >= len) {
        return;
    }
    jint singleValue[1] = {value};
    env->SetIntArrayRegion(array, index, 1, singleValue);
}
